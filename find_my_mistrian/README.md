...
Find My Mistrian is a small quality-of-life mod for Fields of Mistria. It adds a live location readout and a **Locate on map** button to the Relationships page.
...
3. Read their current location on the right page.
4. Choose **Locate on map** to open the correct map region and blink the character's existing map icon for a few seconds.

Unknown or progression-locked characters are not made selectable by the mod. If the vanilla game has no usable current position or does not draw the icon, the mod reports that the character cannot be located.
...
- Mods that completely replace the Relationships or Map menu internals may conflict. Event-based decoration is used instead of replacing vanilla constructors.
- A controller can navigate the existing Relationships list and the added button. MOMI 0.15.1 hotkeys are keyboard-only, so the optional shortcut remains a keyboard binding.
